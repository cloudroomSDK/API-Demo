<template>
  <div>
    <div class="control">
      ...
      <div class="block">
        <div class="text">切片时长:</div>
        <el-select
          v-model="recordConfig.maxFileDuration"
          class="right"
          :disabled="meetingState !== 2 || recordState !== 0"
          placeholder=""
        >
      ...
      <div class="checkboxGroup">
        <el-checkbox
          v-model="recordConfig.onlyMe"
          @change="updateLayout"
          >只录自己</el-checkbox
        >
        <el-checkbox
          v-model="recordConfig.showTime"
          @change="updateLayout"
          >录制时戳</el-checkbox
        >
        <el-checkbox
...
export default {
  ...
  created() {
    ...
    RTCSDK.CloudMixerEvent.callback = this.CloudMixerEvent; // SDK回调接口：通知云端录制文件生成进度
  },
  destroyed() {
    RTCSDK.CreateCloudMixerFailed.callback = null;
    RTCSDK.CloudMixerEvent.callback = null;
  ...
  methods: {
    ...
    clickStartBtn() {
      const { definition, frameRate, onlyMe, maxFileDuration } = this.recordConfig;
      const { W, H, bitRate } = this.definitionConfig[definition];

      const svrPathName = parseTime(new Date(), '/YYYY-MM-DD/YYYY-MM-DD_HH-mm-ss') + `_Web_API_${this.roomId}.mp4`;
      const cfg = {
        mode: 0, // 录制合流模式
        videoFileCfg: {
          svrPathName,
    ...
    // 云端录制文件、云端直播推流信息变化
    CloudMixerEvent(mixerID, state, jsonState) {
      if (mixerID !== this.mixerId) return;
      switch (state) {
        case 1:
        case 8:
        ...
        case 3: {
          const element = this.$createElement(RecordToast, {
            props: {
              fileName: jsonState.outputName,
              jsonState: jsonState,
              state: state,
            },
          });
          this.recordResultToastElement.push(element);
          this.$notify({
            title: jsonState.outputName,
            duration: 0,
            message: element,
          });
          break;
        ...
        default: {
          const idx = this.recordResultToastElement.findIndex((item) => item.child.fileName === jsonState.outputName);
          if (idx > -1) {
            this.recordResultToastElement[idx].child.updateState(state, jsonState);
          }
          break;
        }
      }
